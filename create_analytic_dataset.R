...
dfs[, cutoff := ifelse(cd4 < 60, 0, 1)]

# Confirm that we se different risk of the outcome across the cutoff strata for CD4
...
# See MUCH lower risk among those where CD4 is greater than or equal to 60
...
# differential by CD4.
dfs[, days_c := rweibull(.N, shape = 0.5 + 2.9 * cutoff, scale = ifelse(cutoff == 1, 650, 850))]
